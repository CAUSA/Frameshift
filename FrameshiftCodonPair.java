...
in-frame deletions for Recodon (Frameshift)
	...
	for (int i=1;i<=4;i++){
		...
		for (int j=1;j<=4;j++){
			...
			for (int k=1;k<=4;k++){
				...
				String AA=Frameshift.translate(codon);
				NoCodon++;
			//	System.out.println("Codon"+String.valueOf(NoCodon)+": "+ codon+": "+AA +"-->");
				for (int s=1;s<=4;s++){
				   codon2=codon.substring(1,codon.length());
					...
					String AA2=Frameshift.translate(codon2);
					if (!AA.equals(AA2)){
						outstr="     "+codon2+": "+AA2;
						String SScore=Frameshift.substitution_score(S, AA, AA2);
				...
				for (int l=1;l<=4;l++){
					...
					for (int m=1;m<=4;m++){
						...
						for (int n=1;n<=4;n++){
							...
							String AA1=Frameshift.translate(codon2);
							if (!AA.equals(AA1)){
								outstr="     "+codon2+": "+AA1;
								String SScore=Frameshift.substitution_score(S, AA, AA1);
							//	System.out.println(outstr+": "+String.valueOf(SScore));
								NumSub++;

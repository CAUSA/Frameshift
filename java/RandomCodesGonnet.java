...
public class RandomCodesGonnet{

	public static void main(String[] args) 
	{
		int NumCodes=Integer.parseInt(args[0]); 
		int CodeScores[]=new int[NumCodes+1]; 
				...
				for(int i2=0;i2<4;i2++){
					...
					for(int i3=0;i3<4;i3++){
						...
						for(int i4=0;i4<4;i4++)
						{
							...
							int score;
							if(MatrixIndex(Translate(codon1,code))=='*'||MatrixIndex(Translate(codon2,code))=='*')
									{
								score=0;
									}
							else
							{
								score=ScoringMatrix[MatrixIndex(Translate(codon1,code))][MatrixIndex(Translate(codon2,code))];
							}
						...
						for(int i5=0;i5<4;i5++)
						{
							...
							{
								score1=ScoringMatrix[MatrixIndex(Translate(codon1,code))][MatrixIndex(Translate(codon2,code))];
							}
	...
	System.out.println("\n\nTotal Number of Code: "+String.valueOf(NumCodes)+"\r\n");
		...
		for(int g=0;g<=NumCodes;g++){
			...
			CodeScores[g]+=10000; //Transform the scores to positive integers
		}
	...
	/*	
		...
		*/
		...
		CountSort(CodeScores); //Sort the scores by count sort
		...
		System.out.print(NumCodes-Rank+1);
		System.out.print("/");
	...
	public static int MatrixIndex(char a)
	{   int c;
		switch(a)
		{
		case 'A':c=0;break;
...
public static void CountSort(int[] sortArray){
       int max = sortArray[0];
	   int ArrayLen=sortArray.length;

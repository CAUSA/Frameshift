...
import java.io.*;
import java.util.*;
public class FrameshiftCodonRandom {
	public static String translate(String codon, String []RandomAmino, long RandomSeed){
		String amino="";
	...
	String[]RandomAmino=new String [21];
	String allAAs="CSTPAGNDEQHRKMILVFYW*";
	Random randomNumber = new Random(RandomSeed);
	for (int r=1;r<1000;r++){
		for (int r1=1;r1<=21;r1++){
			RN=Math.abs(random.nextInt())%20;
			RandomAmino[r1]=allAAs.substring(RN,1);
		}				
			for (int M=1;M<=3;M++){
			...
			for ( i=1;i<=4;i++){
				...
				for ( j=1;j<=4;j++){
					...
					for (k=1;k<=4;k++){
						...
						NoCodon++;
						System.out.print("Codon"+String.valueOf(NoCodon)+":"+ codon+":"+ AA1 +"-->");
						for (s=1;s<=4;s++){
